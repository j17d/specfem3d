...
\texttt{utils/small\_utilities/attenuation/conversion\_from\_Qkappa\_Qmu\_to\_Qp\_Qs\_from\_Dahlen\_Tromp\_959\_960.f90}.\newline
...
When you have an existing CUBIT (or similar) mesh stored in SPECFEM3D Cartesian
...
CUBIT (or similar) mesh that this code will read in SPECFEM3D Cartesian format
...
\texttt{utils/small\_utilities/attenuation/}.
...
to SPECFEM3D Cartesian format automatically for you: "run\_cubit2specfem3d.py".
...
that you want to export to SPECFEM3D Cartesian format. These scripts will do the conversion for you automatically except assigning material properties
...
(To see how to use it, see the EXAMPLES specific to the internal mesher in directory \texttt{EXAMPLES/applications/meshfem3D\_examples/}.)
...
To run the mesher on your cluster, we provide some example scripts for you to modify accordingly in \texttt{utils/scripts/Cluster/}.
...
default job submission scripts provided in directory \texttt{utils/scripts/Cluster/}.
...
The present version of SPECFEM3D Cartesian can handle fully saturated porous simulations.
...
The way we prescribe material property for porous material in SPECFEM3D Cartesian depends on wheter the internal mesher \texttt{xmeshfem3D}
...
\end{description}
Using an external mesh (for instance coming from CUBIT/TRELIS), poroelastic materials have the ID number 3, while 1 is acoustic and 2 is elastic (see the example in the package: \texttt{EXAMPLES/applications/homogeneous\_poroelastic}).\newline
