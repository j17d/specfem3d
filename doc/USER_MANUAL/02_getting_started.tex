...
To download the SPECFEM3D Cartesian software package, type this:
\begin{verbatim}
...
You can replace the GNU compilers above (gfortran and gcc) with other compilers if you want to; for instance for Intel ifort and icc use FC=ifort CC=icc instead. Note that MPI must be installed with MPI-IO enabled because parts of SPECFEM3D Cartesian perform I/Os through MPI-IO.
...
\noindent
SPECFEM3D Cartesian now supports CUDA and HIP GPU acceleration.
...
clusters (typically more than $10,000$ MPI processes). SPECFEM3D Cartesian can use the ADIOS library~\cite{Liu2013}
...
As file I/O can be a bottleneck in large-scale simulations, SPECFEM3D Cartesian supports file I/O using the HDF5 format
...
\item [{\texttt{FILE\_PATTERNS}}] In SPECFEM3D Cartesian case, it is \texttt{*.f90* *.F90* *.c* *.cu* *.h*}
